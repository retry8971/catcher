...
# 最终审查 (由 REVIEW 模式填充)
[实施与最终计划的符合性评估总结，是否发现未报告偏差, 不创建文档]
...
## 系统级规范
- **禁止**: 严禁使用复数作为文件名/类名，如`Accounts` 
- **类名**：使用PascalCase，如`CloudAccount`
- **方法名**：使用snake_case，如`create_account`
- **变量名**：使用snake_case，如`account_id`
- **常量名**：使用全大写，如`MAX_RETRY_COUNT`
- **文件名**：使用蛇形命名法，如`cloud_account.py`
- **重构代码**: 禁止每次重构时增加新的签名方法,直接在原方法名上修改参数和类型,然后再去同步修改引用旧签名的地方.
...
## 参考开发文档和规范
- 前端发开参考 @/Users/xcoplit/workspace/nebula/documents/dev-guide/core.md
- 后端开发参考 @/Users/xcoplit/workspace/nebula/documents/dev-guide/frontend.md

## more
- api endpoint: https://n.fee.red/api
- frontend: https://n.fee.red

## MCP
- playwright 请使用headless模式,如遇到访问失败的情况,最多反复尝试最多5次.截屏内容请保存在当前项目的/playwright-sc目录.
- 检查/迁移现有数据库时请使用mongo MCP,禁止编写脚本访问